...
const mime = { '.html':'text/html; charset=utf-8', '.css':'text/css; charset=utf-8', '.js':'text/javascript; charset=utf-8', '.json':'application/json; charset=utf-8', '.png':'image/png', '.webp':'image/webp', '.jpg':'image/jpeg', '.jpeg':'image/jpeg', '.svg':'image/svg+xml' };
...
try {
  ...
  for (const [name, viewport] of targets) {
    ...
    const introProbe = await page.evaluate(() => {
      ...
      return { startVisible:visible(document.getElementById('startScreen')), startButtonText:document.getElementById('startGameBtn')?.textContent?.trim()||'', profileStarted:typeof S==='object'?!!S.profileStarted:null, version:typeof S==='object'?S.version:null, gold:typeof S==='object'?S.gold:null, gems:typeof S==='object'?S.gems:null };
    });
    await page.click('#startGameBtn');
    await page.waitForTimeout(120);
    const starterProbe = await page.evaluate(() => {
      const screen=document.getElementById('starterScreen'), r=screen?.getBoundingClientRect(), s=screen?getComputedStyle(screen):null;
      return { visible:!!screen&&r.width>0&&r.height>0&&s.display!=='none'&&s.visibility!=='hidden', cards:document.querySelectorAll('.starterCard').length, chooseButtons:document.querySelectorAll('.starterChoose').length };
    });
    await page.click('.starterChoose');
    await page.waitForTimeout(300);

    const audit = await page.evaluate(() => {
      const body=document.body, canvas=document.querySelector('#battleCanvas'), get=id=>document.getElementById(id)?.textContent||'';
      const visibleButtons=[...document.querySelectorAll('button')].filter(el=>{const r=el.getBoundingClientRect(),s=getComputedStyle(el);return r.width>0&&r.height>0&&s.display!=='none'&&s.visibility!=='hidden';});
      let balance=null;
      try {
        const levels=[1,10,25,50,100].map(level=>({level,exp:expNeed(level)}));
        const active=typeof calcStats==='function'?calcStats():null;
        balance={ levels, activeStats:active?{attack:active.atk,attackSpeed:active.aspd,crit:active.crit,critDamage:active.critDamage,potentialMainPct:active.potentialPct,potentialAttackPct:active.potentialAttackPct,power:active.power}:null, potentialExample400:400*10, skillCosts:typeof skillUnitCost==='function'?[1,10,25,50].map(lv=>skillUnitCost(0,lv)):[] };
      } catch(error) { balance={error:String(error)}; }
      return { title:document.title, textSample:body.innerText.slice(0,500), scrollWidth:body.scrollWidth, clientWidth:body.clientWidth, horizontalOverflow:body.scrollWidth>body.clientWidth+2, canvas:canvas?{width:canvas.width,height:canvas.height,cssWidth:canvas.getBoundingClientRect().width,cssHeight:canvas.getBoundingClientRect().height}:null, visibleButtonCount:visibleButtons.length, tinyTapTargets:visibleButtons.filter(el=>{const r=el.getBoundingClientRect();return r.width<32||r.height<32;}).map(el=>({text:(el.textContent||'').trim().slice(0,40),width:Math.round(el.getBoundingClientRect().width),height:Math.round(el.getBoundingClientRect().height)})), gold:get('gold'), attack:get('attack'), power:get('power'), profileStarted:!!S.profileStarted, firstCharacter:S.firstCharacter, runtimeSessionActive:!!runtimeSessionActive, balance };
    });
    audit.intro=introProbe; audit.starter=starterProbe; results.views[name]=audit;

    if (name==='desktop') {
      const saveProbe=await page.evaluate(()=>{
        if(typeof save!=='function'||typeof S!=='object') return {supported:false};
        const originalRaw=localStorage.getItem('sproutFinalV19'), probeGold=1234567;
        S.gold=probeGold; save();
        const raw=localStorage.getItem('sproutFinalV19'), parsed=raw?JSON.parse(raw):null;
      ...
      const reloadProbe=await page.evaluate(()=>{
        const originalRaw=sessionStorage.getItem('__saveProbeOriginalV19'), reloadedGold=typeof S==='object'?S.gold:null, reloadedVersion=typeof S==='object'?S.version:null, reloadedProfileStarted=typeof S==='object'?!!S.profileStarted:null, startButtonText=document.getElementById('startGameBtn')?.textContent?.trim()||'';
        if(originalRaw==='__NONE__'||originalRaw===null) localStorage.removeItem('sproutFinalV19'); else localStorage.setItem('sproutFinalV19',originalRaw);
