...
// A personal debug preview can include the current web artwork. Release builds
// retain the verified-assets gate until provenance is documented.
const ledger = JSON.parse(readFileSync(join(root, 'release/asset-ledger.json'), 'utf8'));
mkdirSync(join(out, 'assets'));
const previewArt = process.env.SPROUT_PREVIEW_ART === '1';
for (const asset of ledger.assets.filter(item => previewArt || (item.commercial_rights_verified && item.source_evidence))) {
...
html = html.replace('</head>', `<script>window.SPROUT_VERIFIED_ASSETS_ONLY=${!previewArt}</script>\n<link rel="stylesheet" href="mobile.css" />\n<script type="module" src="mobile.js"></script>\n</head>`);
